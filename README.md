...
- **Time Range Box** — A generalized, duration-based version of a session box: pick any higher timeframe (e.g. 1H) and it draws that period's Open/High/Low/Close as three stacked box segments on your current (lower-timeframe) chart, live as the period forms — an upper wick (the higher of Open/Close, up to the High), a body (Open to Close, more opaque so it stands out), and a lower wick (the Low, up to the lower of Open/Close), the same anatomy as a regular candle. Consecutive periods alternate between two colors so each one is easy to tell apart at a glance, and a lighter full-chart-height tint in the same alternating colors extends behind the candles so the period boundaries stay visible even outside the box itself. Only draws when the selected timeframe is actually higher than your chart's own — e.g. pick 1H while viewing a 5-minute chart, not the other way around. A "Periods to Keep" input controls how many completed boxes stay on the chart before older ones are removed — or check "Keep All Periods" to never trim them (still capped by TradingView's own 500-object drawing limit, shared across all of TradeKit's box/line features). A label showing the period's O/H/L/C as compact text (e.g. `O84.6 H88.5 L80.4 C87.5`), updating live as the period forms, is on by default — anchored just outside whichever corner of the box you pick (Top Left by default, or Top Right/Bottom Left/Bottom Right), with its own font size input.
